...
def generateTemplates(config):
	...
	production = productionTemplate.replace(
		...
		"SCITRAN-CENTRAL-URL",         config['central']['url'])
...
def bootstrapData(config, docker, apiName, mongoName, email):
	...
	container = docker.create_container(
		...
		volumes=['/service/config', '/service/code', '/service/blocks'],
		command=["./bootstrap.py", "dbinitsort", "mongodb://mongo/nims", "/service/code/testdata/", "/service/blocks", "-j", "/service/config/bootstrap.json", "-k"]
	)
	...
	docker.start(container=container["Id"], links={mongoID: "mongo"}, binds={
		os.path.join(os.getcwd(), 'api'):         {'bind': '/service/config', 'ro': False },
		os.path.join(os.getcwd(), 'code'):        {'bind': '/service/code',   'ro': False },
		os.path.join(os.getcwd(), 'data/blocks'): {'bind': '/service/blocks', 'ro': False },
	})
